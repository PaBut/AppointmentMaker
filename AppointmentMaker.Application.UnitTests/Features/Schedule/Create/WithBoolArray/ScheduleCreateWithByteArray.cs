...
namespace AppointmentMaker.Application.UnitTests.Features.Schedule.Create.WithBoolArray;

public class ScheduleCreateWithByteArray
{
    public class CreateScheduleWithTimeIntervalsHandlerTests
    {
        ...
        public async Task ShouldReturnProperResult()
        {
            ...
            bool[] template = options.Value.GetBoolTemplateFromTimeIntervals(intervals);

            ScheduleCreateWithBoolArrayCommandHandler handler
                = new ScheduleCreateWithBoolArrayCommandHandler
                (options, _repository, _unitOfWork);

            var result = await handler.Handle
                (new ScheduleCreateWithBoolArrayCommand(template, Guid.NewGuid().ToString()),
